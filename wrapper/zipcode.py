...
class Zipcode(object): # object's constructor initialised by the zip code
  ...
  def result(self): # returns the APIs results
    ...
    if self.country is not None and self.zip_code is not None: # condition based on if country code drop-down was selected (and the zipcode is not None **avoiding adding str to None)
    ...
    #***for testing print(response.request.url)
